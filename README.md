...
Реализация собственной библиотеки s21_decimal.h на языке C.  

Эта библиотека должна добавить возможность работы с типом «decimal», который отсутствует в стандарте языка. Этот тип критически важен, например, для финансовых расчетов, где недопустимы погрешности вычислений, свойственные типам с плавающей точкой.  
...
## Задание. Реализация функции библиотеки decimal.h
...
- При реализации decimal ориентируйся на описанное выше двоичное представление с целочисленным массивом `bits`, как указано в примере выше. Соблюдай положение разрядов числа в массиве `bits`.
